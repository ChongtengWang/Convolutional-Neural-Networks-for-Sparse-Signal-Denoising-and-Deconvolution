...
for m = 1:1:3
    ...
    %         groundtruth(index)  = 10*randn(1,K);
            ...
            MSE_output = mean(noise.^2);
            MSE_input = mean(input_noise.^2);
            SNR_output(j) = 10*log10(mean(groundtruth.^2)/MSE_output);
            SNR_input(j) = 10*log10(mean(groundtruth.^2)/MSE_input);
...
hold off
legend('Output MSE, sparsity = 85%','Output MSE, sparsity = 90%','Output MSE, sparsity = 95%', ...
       'Input MSE, sparsity = 85%', 'Input MSE, sparsity = 90%', 'Input MSE, sparsity = 95%', 'Location','NorthWest')
xlabel('σ')
ylabel('MSE')
set(gca, 'xtick', [0.1 0.2 0.5 1 2.0])
xlim([min(sigma_set) max(sigma_set)])
